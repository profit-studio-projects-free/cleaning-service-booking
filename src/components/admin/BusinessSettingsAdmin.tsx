...
export function BusinessSettingsAdmin() {
  ...
  async function save() {
    ...
    let res;
    if (existing) {
      res = await supabase.from('business_settings').update(payload).eq('id', existing.id);
    } else {
      res = await supabase.from('business_settings').insert(payload).select().maybeSingle();
      if (!res.error && res.data) setExisting(res.data as BusinessSettings);
    }
    setSaving(false);
    if (res.error) {
