...
export function Booking({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!cancelled) {
        if (e) {
          // If RLS blocks reads on public, slots fall back to working hours only.
          setAppointmentsForDate([]);
        } else {
          setAppointmentsForDate((data ?? []) as unknown as Appointment[]);
        }
  ...
  async function submit() {
    ...
    // Important: insert only — do NOT call .select() after insert for public users.
